...
'''
        ...
        if smooth_training_values is not None:
            ax.plot(list(smooth_training_values[test_term]), color='g', linestyle='-', marker='*',
...
'''
    ...
    training_values_eval = smoothed_training_values if smoothed_training_values is not None else training_values
    test_values_eval = smoothed_test_values if smoothed_test_values is not None else test_values

    if test_forecasts:
        html_string += 'Note that any NaN results will be ignored when calculating standard deviation<p/>\n'

        rmse_html_string, rmse_df = create_html_table(predictor_names, results, test_terms, test_values_eval,
                                                      training_values_eval,
        ...
        abs_error_html_string, abs_error_df = create_html_table(predictor_names, results, test_terms, test_values_eval,
                                                                training_values_eval,
                                                                'Absolute error', extract_mean_absolute_error,
                                                                value_scale=abs_error_value_scale)

        avg_rmse_html_string, avg_rmse_df = create_html_table(predictor_names, results, test_terms, test_values_eval,
                                                              training_values_eval, 'Average RMSE', extract_rmse)
